...
const HomeScreen = ({ onGoToCreateWorkout, onStartWorkout, onLogout, onGoToProfileEdit }) => {
  ...
  // Tradução dos dias da semana (inglês → português)
  ...
  const loadData = async () => {
    try {
      ...
      data.forEach(workout => {
        const day = workout.dayOfWeek || 'Monday'; // fallback
  ...
  const handleStartWorkout = (workout) => onStartWorkout(workout);
          ...
          dayOrder.map(day => {
            ...
            return (
              <View key={day} style={styles.daySection}>
                <Text style={styles.dayTitle}>{dayTranslation[day]}</Text>
                {workouts.map(workout => (
                  <View key={workout.id} style={styles.workoutCard}>
                    <View style={styles.workoutInfo}>
                      <Text style={styles.workoutName}>{workout.name}</Text>
                    ...
                    <TouchableOpacity style={styles.startBtn} onPress={() => handleStartWorkout(workout)}>
                      <Text style={styles.startText}>INICIAR</Text>
                    </TouchableOpacity>
                  </View>
                ))}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#000', paddingTop: 60 },
  editProfileBtn: { position: 'absolute', top: 60, right: 20, zIndex: 10 },
  editProfileText: { color: '#9D4EDD', fontSize: 18, fontWeight: '600' },
  header: { paddingHorizontal: 20 },
  welcome: { color: '#AAA', fontSize: 18 },
  name: { color: '#FFF', fontSize: 32, fontWeight: 'bold', marginTop: 4 },
  section: { flex: 1, paddingHorizontal: 20, marginTop: 30 },
  ...
  workoutName: { color: '#FFF', fontSize: 18, fontWeight: 'bold' },
  workoutCount: { color: '#00FFCC', fontSize: 14, marginTop: 4 },
  startBtn: { backgroundColor: '#00FFCC', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 25 },
  startText: { color: '#000', fontWeight: 'bold', fontSize: 15 },
  fab: { position: 'absolute', bottom: 30, right: 20, backgroundColor: '#9D4EDD', width: 60, height: 60, borderRadius: 30, justifyContent: 'center', alignItems: 'center', elevation: 8 },
  fabText: { color: '#FFF', fontSize: 32, fontWeight: 'bold' },
  logoutBtn: { position: 'absolute', bottom: 30, left: 20, padding: 10, zIndex: 10 },
  logoutText: { color: '#FF4444', fontSize: 18, fontWeight: '600' },
